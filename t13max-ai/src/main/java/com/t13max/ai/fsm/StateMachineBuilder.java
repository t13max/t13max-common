 ...
 */
public class StateMachineBuilder<T, S extends IState<T>> {
    ...
    private StateMachineBuilder(int type) {
        if (type == TYPE_HFSM)
            this.stateMachine = new HierarchicStateMachine<>();
    }

    public static <T, S extends IState<T>> StateMachineBuilder<T, S> HFSMBuilder() {
